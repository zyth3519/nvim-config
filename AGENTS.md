...
- `lua/config/keymaps/project/init.lua`: setup and `:ProjectRunRedetect`
- `lua/config/keymaps/project/loader.lua`: rule loading and project resolution
- `lua/config/keymaps/project/runtime.lua`: `<leader>rN` / `<leader>rrN` generation
...
When behavior changes, update `README.md` and the nearest module README together, especially for `lua/config/keymaps/project/README.md`. Avoid machine-specific paths and secrets. Prefer Neovim APIs such as `vim.fn.stdpath()`, `vim.fs`, and `vim.uv` over ad hoc shell assumptions.
